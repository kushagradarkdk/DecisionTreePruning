...
def label_counts(rows):
    ...
    maxLabel = sorted(counts, key=counts.__getitem__, reverse=True)
    return maxLabel[0]
...
def print_tree(node, depth=0, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print(spacing + "Node id = "+str(node.id) + " Predict "
              + str(node.predictions) + " Class = " + str(node.predictedClassLabel) )
        return
